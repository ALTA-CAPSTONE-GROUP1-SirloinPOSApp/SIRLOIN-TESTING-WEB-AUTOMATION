Feature: Product Tenant
  ...
  Scenario: Verify information profile tenant same like registration
    ...
    And user click profile tenant menu
    Then user should be on "Profil Tenant" page
    And verify toko have name "Toko Berkah Mukhlis"
    And verify email have value "mukhlisanshori321@gmail.com"
    And verify nomor telephone is "0825123123"
    And verify address is "jl malang indonesia raya"
  ...
  Scenario: Verify edit fitur phone number can be used
    ...
    And user click button edit profile
    And verify user on "Update Profil Tenant"
    Then user update phone number with "082257985887"
    And user click save button
    Then user verify phone number updated with "082257985887" on profile tenant page
  ...
  Scenario: Verify edit fitur alamat can be used
    ...
    And user click button edit profile
    And verify user on "Update Profil Tenant"
    Then user update alamat with "JL Kol Sugiono Mergosono Malang Indonesia"
    And user click save button
    Then user verify alamat updated with "JL Kol Sugiono Mergosono Malang Indonesia" on profile tenant page
